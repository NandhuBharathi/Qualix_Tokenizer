...
void PrintHex(std::string_view text)
{
    usize offset = 0;
    ...
    {
        auto decoded =
            Utf8::Decode(text.substr(offset));
...
int main()
{
    const char* path =
        ...
        "tools/unicode/data/NormalizationTest.txt";
    ...
    while (std::getline(file, line))
    {
        ...
        for (const auto& check : test_checks)
        {
            ++checks;
            ...
            auto actual = Normalizer::Normalize(
                *check.input,
                NormalizationForm::NFD
            );
